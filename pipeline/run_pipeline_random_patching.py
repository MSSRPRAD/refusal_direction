...
def run_pipeline_patching_modified(model_path):
    ...
    first_fifty_direction = torch.rand_like(direction)
    first_fifty_direction = first_fifty_direction / torch.sqrt(torch.sum(first_fifty_direction**2))
    # print(first_fifty_direction.shape)

    baseline_fwd_pre_hooks, baseline_fwd_hooks = [], []
    ablation_fwd_pre_hooks, ablation_fwd_hooks = get_all_direction_ablation_hooks_first_k(model_base, first_fifty_direction)
    actadd_fwd_pre_hooks, actadd_fwd_hooks = [(model_base.model_block_modules[layer], get_activation_addition_input_pre_hook_first_k(vector=first_fifty_direction, coeff=-1.0))], []
    ...
    for dataset_name in cfg.evaluation_datasets:
        ...
        generate_and_save_completions_for_dataset(cfg, model_base, ablation_fwd_pre_hooks, ablation_fwd_hooks, 'first_fifty_ablation', dataset_name)
        generate_and_save_completions_for_dataset(cfg, model_base, actadd_fwd_pre_hooks, actadd_fwd_hooks, 'first_fifty_actadd', dataset_name)
    ...
    for dataset_name in cfg.evaluation_datasets:
        ...
        evaluate_completions_and_save_results_for_dataset(cfg, 'first_fifty_ablation', dataset_name, eval_methodologies=cfg.jailbreak_eval_methodologies)
        evaluate_completions_and_save_results_for_dataset(cfg, 'first_fifty_actadd', dataset_name, eval_methodologies=cfg.jailbreak_eval_methodologies)
    ...
    actadd_refusal_pre_hooks, actadd_refusal_hooks = [(model_base.model_block_modules[layer], get_activation_addition_input_pre_hook(vector=first_fifty_direction, coeff=+1.0))], []
    generate_and_save_completions_for_dataset(cfg, model_base, actadd_refusal_pre_hooks, actadd_refusal_hooks, 'first_fifty_actadd', 'harmless', dataset=harmless_test)
    ...
    evaluate_completions_and_save_results_for_dataset(cfg, 'first_fifty_actadd', 'harmless', eval_methodologies=cfg.refusal_eval_methodologies)
    ...
    evaluate_loss_for_datasets(cfg, model_base, ablation_fwd_pre_hooks, ablation_fwd_hooks, 'first_fifty_ablation')
    evaluate_loss_for_datasets(cfg, model_base, actadd_fwd_pre_hooks, actadd_fwd_hooks, 'first_fifty_actadd')

if __name__ == "__main__":
    args = parse_arguments()
    run_pipeline_patching_modified(model_path=args.model_path)